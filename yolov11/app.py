...
def load_model():
    """Load YOLO model."""
    try:
        logging.info(f"🔍 Checking if model exists: {weights_path}")
...
async def fetch_file_from_mongo(file_id):
    ...
    logging.info(f"🔍 Searching for file ID: {file_id}")
    try:
        object_id = ObjectId(file_id)
    except Exception as e:
        logging.error(f"❌ Invalid ObjectId format: {e}")
        return None

    try:
        file = await collection.find_one({"_id": object_id})
        if file:
            logging.info(f"✅ File Found: {file.get('filename', 'Unknown')}")
            file_data = file.get("data")
            if not file_data:
                logging.error("❌ File data is missing in MongoDB")
                return None
            return file_data
        else:
            logging.error("❌ File not found in MongoDB - Verify ID and Storage")
            return None
...
async def process_image(file_id, model):
    ...
    if not image_data:
        logging.error("❌ Image data is missing")
        return
...
async def save_processed_file(file_id, processed_data):
    ...
    try:
        ...
        if result.modified_count > 0:
            logging.info(f"✅ Processed file saved to MongoDB for file ID: {file_id}")
        else:
            logging.error("❌ Failed to save processed file in MongoDB")
    except mongo_errors.PyMongoError as e:
        logging.error(f"❌ MongoDB error while saving processed file: {e}")
...
async def process_video(file_id, model):
    ...
    if not file_data:
        logging.error("❌ Video data is missing in MongoDB")
        return
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        logging.error("Usage: python app.py <file_id> <file_type>")
        sys.exit(1)
    ...
    if file_type.startswith("image"):
        ...
        logging.error("❌ Invalid file type. Use 'image' or 'video'.")
